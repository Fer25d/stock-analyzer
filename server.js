...
// ── /api/indices  TradingView Scanner → Yahoo fallback ──
const TV_INDICES = [
...
// Cache para no pegar a TV en cada request (actualiza cada 3s)
let _idxCache = null;
let _idxCacheTime = 0;
const IDX_CACHE_MS = 3000;

app.get('/api/indices', async (req, res) => {
  try {
    const now = Date.now();
    if (_idxCache && (now - _idxCacheTime) < IDX_CACHE_MS) {
...
app.get('/api/finviz', async (req, res) => {
  ...
  try {
    const r = await fetch(
      `https://finviz.com/quote.ashx?t=${sym}&ty=c&ta=1&p=d`,
